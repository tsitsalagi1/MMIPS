...
type Preview = {
  profile: { id: string; slug: string; name: string; urgency_level: string; public_map_label: string };
  matchedCount: number;
  canSend: boolean;
};
...
export default function AdminUrgentAlerts() {
  ...
  useEffect(() => {
    let alive = true;
    supabase.auth.getSession().then(({ data }) => { if (alive) setSessionToken(data.session?.access_token ?? null); });
  ...
  async function search(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const response = await fetch(`/api/admin/profiles?${params.toString()}`, { headers: { Authorization: `Bearer ${sessionToken}` } });
  ...
  async function choose(profile: AdminProfile) {
    ...
    try {
      ...
      const response = await fetch(`/api/admin/alerts/urgent?${params.toString()}`, { headers: { Authorization: `Bearer ${sessionToken}` } });
      const data = await response.json();
  ...
  async function sendAlert() {
    ...
    const ok = window.confirm(`Send this approved urgent public alert to ${preview.matchedCount} matched confirmed subscriber${preview.matchedCount === 1 ? "" : "s"}?`);
    if (!ok) return;
    setLoading(true);
    setMessage("");
    try {
  ...
  return (
    <section className="container section correction-admin-section" aria-labelledby="urgent-alert-admin-heading">
      ...
      <p className="lead">Send a geographically targeted urgent email only after the public profile and its approximate public map point are approved. Subscriber ZIP/radius preferences remain private.</p>

      <section className="notice warning">
        <strong>Human approval is required.</strong>
        <p>A raw submission never sends a public alert automatically. The profile must be published, marked <em>Urgent public awareness</em>, have an approved approximate public map point, and pass this final moderator confirmation.</p>
      ...
      <form className="card admin-controls" onSubmit={search}>
        <label>Find a published profile
          <input type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Name, slug, Tribe, agency, or public area" minLength={2} />
        </label>
          ...
          {profiles.map((profile) => {
            ...
            return (
              <article className="card" key={profile.id}>
                ...
                <p className="muted">{person?.tribal_affiliation || "Tribal affiliation not listed"} · {profile.status} · {profile.urgency_level || "standard"}</p>
                <button type="button" className="secondary" onClick={() => choose(profile)} disabled={loading}>Preview urgent audience</button>
              </article>
            );
          })}
        </div>
      ...
      {selected && preview ? (
        <section className="card stack">
          ...
          <p><strong>Approved public-awareness area:</strong> {preview.profile.public_map_label}</p>
          <p><strong>Confirmed subscribers matched:</strong> {preview.matchedCount}</p>
          <p><strong>Urgency:</strong> {preview.profile.urgency_level}</p>
          {!preview.canSend ? <p className="notice warning">This profile is not currently eligible. It must be approved, published, and marked Urgent public awareness.</p> : null}
          <label>Type <strong>SEND URGENT ALERT</strong> to confirm
            <input value={confirmation} onChange={(event) => setConfirmation(event.target.value)} autoComplete="off" />
          </label>
          <button type="button" onClick={sendAlert} disabled={loading || !preview.canSend || confirmation.trim() !== "SEND URGENT ALERT"}>{loading ? "Processing…" : `Send to ${preview.matchedCount} matched subscriber${preview.matchedCount === 1 ? "" : "s"}`}</button>
        </section>
      ) : null}
