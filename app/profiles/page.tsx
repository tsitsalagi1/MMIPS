...
export default async function ProfilesPage() {
  ...
  return (
    <main className="container section">
      ...
      <div className="card" style={{ margin: "20px 0" }}>
        <form className="form">
          <label>Search by name, city, Tribe, agency, or NamUs number
            <input placeholder="Search MMIPS public profiles" />
          </label>
      ...
      {profiles.length ? (
        profiles.map((item) => <CaseCard key={item.id} item={item} />)
      ) : (
        <div className="card calm-panel" style={{ marginTop: "22px" }}>
          <h2>No public profiles are listed yet.</h2>
          <p className="text-measure">Profiles will appear here only after review and publication. MMIPS does not show private submissions, unreviewed reports, or information that has not been approved for public awareness.</p>
          <p className="muted">Families and authorized contacts can submit information for review or request corrections/removal at any time.</p>
        </div>
      )}
    </main>
  );
