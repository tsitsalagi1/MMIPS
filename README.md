...
This update adds optional automated email confirmations for submitted information, correction/removal requests, reviewer updates, profile publication, and public-profile removal.

Add these Vercel environment variables after verifying a sending domain with your email provider:

```text
RESEND_API_KEY=
EMAIL_FROM=MMIPS <notifications@mmips.com>
EMAIL_REPLY_TO=contact@mmips.com
```

If `RESEND_API_KEY` is missing, MMIPS will still save submissions and correction/removal requests, but confirmation emails will be skipped and logged server-side. Do not use Cloudflare Email Routing alone for outbound email; it is for inbound forwarding.

The admin correction/removal section now includes a **Remove public profile** button. This sets the linked public profile to hidden and removes `published_at`, so the profile is removed from public views while preserving the internal record and audit trail.
