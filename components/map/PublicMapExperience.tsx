...
export default function PublicMapExperience({ points, availability }: Props) {
  ...
  async function focusZip(event: FormEvent<HTMLFormElement>) {
    ...
    setZipLookup({ kind: "loading", message: `Locating ZIP code ${zip}…` });
    try {
      ...
      const body = await response.json() as { zip?: string; latitude?: number; longitude?: number; error?: string };
      ...
      setSelectedId(null);
      setZipLookup({ kind: "success", message: `Map centered near ZIP code ${body.zip || zip}.` });
    } catch {
  ...
  return <section className={styles.shell} aria-label="Public map explorer">
    ...
    <form className={styles.zipSearch} onSubmit={focusZip} aria-label="Zoom public map to a ZIP code">
      <label htmlFor="map-zip-code">Zoom map to a ZIP code</label>
      <div className={styles.zipRow}>
        <input
          ...
          placeholder="74464"
        />
        <button type="submit" className="button" disabled={zipLookup.kind === "loading"}>{zipLookup.kind === "loading" ? "Locating…" : "Go to ZIP"}</button>
      </div>
      <p id="map-zip-help" className="muted">Enter a U.S. ZIP code to move the map to that general area. MMIPS does not save this ZIP search as a case location.</p>
    ...
    <p className={styles.live} role="status" aria-live="polite">{filtered.length} public map result{filtered.length === 1 ? "" : "s"} available for these filters.</p>
    ...
    <aside className={styles.mapRegion} aria-labelledby="visual-map-heading">
      ...
      {availability === "available" && hasMapPoints ? <MapLibreRenderer points={filtered} onSelect={setSelectedId} focusTarget={mapFocus} /> : null}
