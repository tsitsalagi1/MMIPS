...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!caseId) return NextResponse.json({ ok: false, message: "Choose a published profile first." }, { status: 400 });
    const loaded = await loadTarget(admin, caseId);
    if ("error" in loaded) return NextResponse.json({ ok: false, message: loaded.error }, { status: 409 });
    ...
    return NextResponse.json({
      ...
      canSend: loaded.profile.review_status === "approved" && Boolean(loaded.profile.published_at) && loaded.profile.urgency_level === "urgent_public_awareness"
    });
  } catch {
    return safeApiError({ code: "urgent_alert_preview_failed", message: "Could not preview the urgent alert audience." });
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!caseId) return NextResponse.json({ ok: false, message: "Choose a published profile first." }, { status: 400 });
    if (confirmation !== "SEND URGENT ALERT") return NextResponse.json({ ok: false, message: "Type SEND URGENT ALERT to confirm this community-wide action." }, { status: 400 });

    const loaded = await loadTarget(admin, caseId);
    if ("error" in loaded) return NextResponse.json({ ok: false, message: loaded.error }, { status: 409 });
    if (loaded.profile.review_status !== "approved" || !loaded.profile.published_at) return NextResponse.json({ ok: false, message: "Urgent alerts can be sent only for approved published profiles." }, { status: 409 });
    if (loaded.profile.urgency_level !== "urgent_public_awareness") return NextResponse.json({ ok: false, message: "Mark the public profile as Urgent public awareness before sending an urgent alert." }, { status: 409 });

    // Synthetic rehearsal lock: do not send real-case alerts while the release rehearsal is active.
    if (!loaded.target.title.startsWith("MMIPS TEST PERSON")) return NextResponse.json({ ok: false, message: "Real-person urgent alerts are locked while the synthetic launch rehearsal is active." }, { status: 423 });
