...
export async function sendUrgentCommunityAlert(target: UrgentAlertTarget, actorId: string) {
  ...
  if (existing) {
    eventId = existing.id;
    matchedCount = Number.isInteger(existing.matched_count) && existing.matched_count >= 0 ? existing.matched_count : 0;
    alertTitle = typeof existing.title === "string" && existing.title ? existing.title : target.title.slice(0, 160);
    alertPublicUrl = typeof existing.public_url === "string" && existing.public_url ? existing.public_url : publicUrl;
    const { error } = await client.from("urgent_alert_events").update({ status: "sending", approved_by: actorId, updated_at: new Date().toISOString() }).eq("id", eventId);
    ...
    const { data: eventRow, error: eventError } = await client.from("urgent_alert_events").insert({
  ...
  const plannedDeliveries: PersistedDelivery[] = (plannedRows ?? [])
    .filter((row) => typeof row.id === "string" && typeof row.subscriber_id === "string" && isDeliveryStatus(row.delivery_status))
    .map((row) => ({ id: row.id, subscriber_id: row.subscriber_id, delivery_status: row.delivery_status }));
  ...
  for (const delivery of plannedDeliveries) {
    ...
    if (!subscriber) {
      await store.updateDelivery(delivery.id, "failed_final", { failureCode: "subscriber_no_longer_eligible" });
      continue;
    }
    const message = buildPublicAlertEmail({
      title: `URGENT COMMUNITY ALERT — ${alertTitle}`,
    ...
    await sendClaimedAlert(store, delivery, { ...message, to: subscriber.email_normalized, subject: "Urgent MMIPS community alert" }, { send: sendTransactionalEmail });
  }
  ...
  const finalStatuses = (finalRows ?? []).map((row) => row.delivery_status).filter(isDeliveryStatus);
  const summary = summarizeUrgentDeliveryState(finalStatuses, matchedCount);
  const completedAt = new Date().toISOString();
  const { error: summaryError } = await client.from("urgent_alert_events").update({
  ...
  return { matched: matchedCount, sent: summary.sent, failed: summary.failed, duplicate: false };
}
